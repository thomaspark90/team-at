  ...
  return (
    <div className="space-y-3">
      {backgrounds.length === 0 && !uploading && (
        <p className="text-body text-gray-400">아직 배경이 없습니다. + 버튼으로 업로드하세요.</p>
      )}
      <div className="flex flex-wrap gap-2">
        {backgrounds.map((bg) => (
          <div key={bg.url} className="relative group">
            ...
            <button
              ...
              className="absolute -top-1 -right-1 w-4 h-4 bg-destructive text-white rounded-full text-caption leading-none hidden group-hover:flex items-center justify-center"
            >
              ×
            </button>
          </div>
        ...
        <button
          ...
          disabled={uploading}
          className="w-16 h-16 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:border-accent hover:text-accent transition text-title"
        >
          {uploading ? <span className="text-caption">...</span> : '+'}
        </button>
      </div>
      <input
        ref={fileRef}
