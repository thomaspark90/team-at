  ...
  return (
    <div className="rounded-2xl border border-border bg-card px-5 py-4">
      ...
      <p className="mt-0.5 text-[12px] text-muted-foreground">
        사람별로 <strong>스탭밀</strong>(송금 요청) · <strong>가든서비스</strong>(원두 재고 20%·소진) 알림을
      ...
      <div className="mt-3 flex flex-col gap-1.5">
        ...
        {rows.map((r) => (
          <div key={r.email} className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5">
            ...
            <button onClick={() => toggle(r, 'transfer')} disabled={busy} className={topicBtn(r.transfer)} title="스탭밀 송금 요청 알림 켜기/끄기">
              스탭밀
            </button>
            <button onClick={() => toggle(r, 'stock')} disabled={busy} className={topicBtn(r.stock)} title="가든서비스 원두 재고 알림 켜기/끄기">
              가든서비스
            </button>
            <button
              onClick={() => {
