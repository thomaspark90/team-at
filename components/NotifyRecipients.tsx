...
export default function NotifyRecipients({ initial }: { initial: RecipientRow[] }) {
  ...
  return (
    <div>
      <h2 className="m-0 text-title font-medium">알림 수신자 관리</h2>
      ...
      <div className="mt-3 flex flex-col gap-3">
        {rows.length === 0 && (
          <span className="text-body text-amber-600">수신자가 없어요 — 기본값(대표)으로 발송돼요.</span>
        )}
        {rows.map((r) => (
          <div key={r.email} className="flex flex-wrap items-center gap-1.5 rounded-lg bg-muted/40 px-3 py-1.5">
            <span className="min-w-0 flex-1 truncate text-body">{r.email}</span>
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-body text-foreground outline-none focus:border-foreground/40"
        />
        <button
          onClick={() => input.trim() && call('POST', { email: input.trim() })}
          disabled={busy || !input.trim()}
          className="rounded-lg bg-foreground px-4 py-2 text-body font-medium text-background disabled:opacity-60"
        >
          추가
        </button>
      </div>
