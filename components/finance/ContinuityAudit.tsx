...
export default function ContinuityAudit({ brand }: { brand: Brand }) {
  ...
  return (
    <details>
      ...
      <button onClick={run} disabled={loading} className="ta-btn-primary text-body">
        ...
        <div className="mt-8 flex flex-col gap-6">
          {results.map((r) => (
            <div key={r.bank} className="rounded-md bg-muted/40 p-3">
              ...
              {r.rows === 0 ? (
                <p className="mb-0 mt-1 text-body text-muted-foreground">저장된 거래가 없어요.</p>
              ) : !r.reliable ? (
                <p className="mb-0 mt-1 text-body text-muted-foreground">
                  끊김이 너무 많아 판정하지 못했어요 — 여러 계좌가 섞였거나, 시간 정보가 없는 파일이라 같은 날
                  거래의 순서가 뒤섞였을 수 있어요.
                </p>
              ) : r.breaks.length === 0 ? (
                <p className="mb-0 mt-1 text-body text-emerald-600">✓ 전 기간 잔액이 이어져요 — 누락 구간 없음</p>
              ) : (
                <ul className="mb-0 mt-4 flex list-none flex-col gap-3 p-0">
                  {r.breaks.map((b, i) => (
                    <li key={i}>
                      <button
                        ...
                        className="w-full rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-left text-body transition-colors hover:border-amber-600"
                      >
                  ...
                  {r.breaks.length >= 100 && (
                    <li className="text-body text-muted-foreground">… 끊김이 100곳을 넘어 이하 생략</li>
                  )}
                </ul>
              )}
            </div>
