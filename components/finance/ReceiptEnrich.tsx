...
export default function ReceiptEnrich() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="text-title text-foreground">🧾 쿠팡 영수증으로 지출 자료 세분화</h2>
      ...
      {preview && (
        <>
          ...
          {preview.matched === 0 && (
            <div className="rounded-md bg-muted p-3 text-body text-muted-foreground">
...
function Td({ children, right, mono, muted }: { children: React.ReactNode; right?: boolean; mono?: boolean; muted?: boolean }) {
  return (
    <td className={`px-3 py-2 text-body ${right ? 'whitespace-nowrap text-right' : 'text-left'} ${mono ? 'tabular' : ''} ${muted ? 'text-muted-foreground' : 'text-foreground'}`}>{children}</td>
  );
}
