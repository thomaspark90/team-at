...
export default function NaverpayConfig() {
  ...
  return (
    <section>
      <h2 className="m-0 text-title font-medium">네이버페이 자동 수집 주소</h2>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <input
          ...
          className="min-w-[260px] flex-1 rounded-xl border border-border bg-background px-3.5 py-2.5 font-mono text-body"
        />
        <button
          onClick={() => save()}
          disabled={saving || !dirty}
          className="rounded-xl bg-foreground px-4 py-2.5 text-body font-medium text-background disabled:opacity-50"
        >
          {saving ? '저장 중…' : '저장'}
        </button>
        {defaultUrl && savedUrl !== defaultUrl && (
          <button
            ...
            disabled={saving}
            className="rounded-xl border border-border px-3 py-2.5 text-body text-muted-foreground hover:text-foreground disabled:opacity-50"
          >
            기본값 복원
          </button>
        )}
      ...
      {notice && <p className="mt-2 text-body" style={{ color: 'hsl(var(--number-colored))' }}>{notice}</p>}
      {error && <p className="mt-2 text-body text-destructive">{error}</p>}
    </section>
  );
}
