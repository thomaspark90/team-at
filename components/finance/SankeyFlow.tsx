    ...
    <div className="flex flex-col gap-4">
      ...
      {!hasData ? (
        <div className="mx-auto my-[60px] text-center text-muted-foreground">
          ...
          <p className="m-0 text-body">이 기간에는 분류된 거래가 없어요.</p>
        </div>
      ) : (
      ...
      <p className="m-0 text-caption leading-[1.6] text-muted-foreground">
        {balanced
...
function Card({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="ta-card min-w-[150px] p-[14px_18px]">
      <div className="mb-1.5 text-caption tracking-[0.02em] text-muted-foreground">{label}</div>
      <div className="tabular text-display font-extrabold" style={{ color }}>
        {value}
      </div>
    </div>
  );
