...
export default function ReviewSalesReport({ data }: { data: StoreReviewSales[] }) {
  ...
  return (
    ...
    <section className="py-[54px]">
      <h2 className="m-0 text-title font-medium">리뷰 × 매출 — 주간 흐름</h2>
          ...
          return (
            <div key={s.store} className="rounded-xl bg-muted/40 p-4">
              <div className="flex items-baseline justify-between gap-2">
                <p className="m-0 text-body font-medium">{storeLabel(s.store)}</p>
              ...
              <div className="mt-1 flex gap-1">
                {recent.map((w) => (
                  <span
                    key={w.week}
                    className={`min-w-0 flex-1 text-center text-caption ${w.reviews > 0 ? 'font-medium text-foreground' : 'text-muted-foreground'}`}
                  >
