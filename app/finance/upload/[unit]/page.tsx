...
//   지점 귀속은 자료 분류(지점 지정·건별 분할)에서 확정한다.
              ...
              <p className="mt-1 text-[13px] text-muted-foreground">
                {unit.store ? (
                  <>
                    ...
                    <Link href={`/finance/classify?unit=${unit.id}`} className="underline">자료 분류</Link>에서 지점을
                    지정하거나 건별 분할로 나눠요.
                  </>
                ) : (
                  <>
                    이 페이지에서 올리는 모든 자료는 <b>스탭밀</b> 회계로 들어가요. 분류는{' '}
                    <Link href="/finance/classify?unit=staffmeal" className="underline">자료 분류</Link>에서 해요.
                  </>
                )}
              </p>
            </div>
