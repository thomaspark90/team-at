...
import { notifyGrindRequest, recipientEmails } from '@/lib/notify';
...
// 분쇄도 요청을 받을 수 있는 담당자 목록(원두 수신자) — 설정 화면의 담당자 선택용
...
export async function POST(req: Request) {
  ...
  // 담당자 지정 시 원두 수신자 목록과 교집합만 허용 — 임의 주소로의 발송 방지
  const allowed = await recipientEmails(supabase, 'stock');
  const picked = (Array.isArray(body.emails) ? body.emails : [])
    .map((e: unknown) => String(e).trim().toLowerCase())
    .filter((e: string) => allowed.includes(e));
  const emails = picked.length > 0 ? picked : undefined; // 미지정 → 원두 수신자 전체
  ...
  for (const storeId of stores) {
    ...
    await logActivity(
      ...
      user,
      '가든서비스 분쇄도 측정 요청',
      `${storeLabel} · ${detail}` + (emails ? ` · 담당 ${emails.join(', ')}` : ' · 원두 수신자 전체')
    );
  }
  return NextResponse.json({ ok: true, sentTo: emails ?? allowed, stores });
}
