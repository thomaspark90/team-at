...
export async function POST(req: Request) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to,
      subject: `[쿠팡 수집 실패] ${reason}`,
      html: `
      <div style="font-family:sans-serif;font-size:14px;line-height:1.7">
        <p>오늘 쿠팡 지출 자동 수집이 실패했어요.</p>
        <p><strong>사유:</strong> ${reason}</p>
        ${detail ? `<p style="color:#666">${detail}</p>` : ''}
        ${isSession ? `
        <p><strong>복구 방법 (약 30초):</strong></p>
        <ol>
          <li>브라우저에서 <a href="https://mc.coupang.com/ssr/desktop/order/list">쿠팡 주문목록</a> 접속(로그인)</li>
          <li>개발자도구(⌥⌘I) → Network → 아무 요청 우클릭 → Copy → <strong>Copy as cURL</strong></li>
          <li>Mac의 <code>~/Projects/coupang-export/쿠키갱신.command</code> 더블클릭</li>
        </ol>
        <p>복구하면 놓친 기간까지 자동으로 따라잡아요.</p>` : `
        <p>반복되면 Claude에게 "쿠팡 수집 실패 확인해줘"라고 요청하세요. (로그: ~/Projects/coupang-export/out/sync.log)</p>`}
      </div>`,
    }),
  });
