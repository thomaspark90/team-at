...
export async function POST(req: Request) {
  ...
  let body: { ym?: string; action?: string; brand?: string };
  try {
    body = await req.json();
  } catch {
  ...
  // 확정은 (ym, brand) 단위 — 한쪽 브랜드 지연이 다른 쪽 마감을 막지 않는다(미지정=garden, 구버전 호환)
  const brand = body.brand === 'staffmeal' ? 'staffmeal' : 'garden';
  ...
  if (action === 'confirm') {
    ...
    const { error } = await supabase
      ...
      .from('monthly_close')
      .upsert(
        { ym, brand, status: 'confirmed', confirmed_by: user.id, confirmed_at: now, updated_at: now },
        { onConflict: 'ym,brand' }
      );
    if (error) return NextResponse.json({ error: `확정 실패: ${error.message}` }, { status: 500 });
    await logActivity(supabase, user, '월 확정', `${ym} [${brand}]`);
    return NextResponse.json({ ym, brand, status: 'confirmed', confirmed_at: now });
  }
  ...
  const { error } = await supabase
    ...
    .from('monthly_close')
    .upsert(
      { ym, brand, status: 'open', confirmed_by: null, confirmed_at: null, updated_at: now },
      { onConflict: 'ym,brand' }
    );
  if (error) return NextResponse.json({ error: `재오픈 실패: ${error.message}` }, { status: 500 });
  await logActivity(supabase, user, '월 확정 재오픈', `${ym} [${brand}]`);
  return NextResponse.json({ ym, brand, status: 'open' });
}
