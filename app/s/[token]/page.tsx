    ...
    >
      {share ? (
        <div className="ta-card bg-background" style={{ width: '100%', maxWidth: 360, textAlign: 'center', padding: '36px 24px' }}>
          ...
          <p className="text-display text-foreground" style={{ margin: '12px 0 4px' }}>
            {share.bean}
          </p>
          <p className="tabular text-foreground" style={{ fontSize: 28, fontWeight: 700, margin: '8px 0' }}>
            {won(share.price)}
          </p>
          <p className="text-body text-muted-foreground" style={{ marginTop: 12 }}>
            {fmtDate(share.decidedAt)} 책정 · team at
          </p>
        </div>
      ) : (
        <div className="ta-card bg-background" style={{ width: '100%', maxWidth: 360, textAlign: 'center', padding: '36px 24px' }}>
          <p className="text-title text-foreground" style={{ marginBottom: 6 }}>링크를 찾을 수 없어요</p>
          <p className="text-body text-muted-foreground">주소가 잘못되었거나 만료된 링크일 수 있습니다.</p>
        </div>
      )}
    </main>
  );
