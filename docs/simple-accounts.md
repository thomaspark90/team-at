...
## 교육 코멘트 (2026-09-09)
- `teaching_slot_comments`(shift·hour·참여 스탭 1행): 진행 단계(1 처음 접함·2 연습 필요·3 혼자 가능)·다룬 주제·코멘트·작성자.
- **쓰기**: 대표 + 그 일정의 티칭 스태프 본인(`PUT /api/garden-teaching/shifts/comments`). 참여 인원(칸 지정 ∪ 일정 지정)에게만.
- **읽기**: 대표 + `profiles.can_view_comments`(설정 › 간편 계정 › 코멘트 열람 체크) + 자기 일정의 티칭 스태프. 그 외엔 API 가 comments 를 비운다.
