...
export type MyAccess = { mine: string[] | null; sections: string[] | null; isAdmin: boolean };
...
export function fetchMyAccess(): Promise<MyAccess> {
  ...
  const promise = fetch('/api/garden-tab-access?scope=mine', { cache: 'no-store' })
    .then((r) => (r.ok ? r.json() : {}))
    .then((j: { mine?: string[] | null; sections?: string[] | null; isAdmin?: boolean }) => ({
      mine: j?.mine ?? null,
    ...
    .catch(() => {
      ...
      return { mine: null, sections: null, isAdmin: false }; // 조회 실패 시 막지 않는다 (내부 도구)
    });
