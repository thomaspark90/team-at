...
JSON 으로만 출력해라: {"issue":true|false,"issue_note":"..."}`;
};
...
export async function classifyIssue(
  ...
  apiKey: string,
): Promise<{ issue: boolean; note: string | null; model: string } | null> {
  ...
  for (const model of MODELS) {
    try {
      ...
      let parsed: { issue?: unknown; issue_note?: unknown };
      try {
      ...
      return { issue: parsed.issue, note: parsed.issue && note ? note : null, model };
    } catch {
      /* 다음 모델로 폴백 */
    }
  }
